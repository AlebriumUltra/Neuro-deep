# -*- coding: utf-8 -*-
"""
Created on Sat Apr 17 03:59:46 2021
...
def train_stochastic(x_values, target, learning_rate, shuffle = False):
    if(shuffle != True):
        for i in range(0, len(x_values)):
            rand = np.random.randint(0, len(x_values))
            output = feed_forward(x_values[rand].reshape(1, -1))
            backward(learning_rate, target[rand].reshape(1, -1), output[2], output)
    else:
        for i in range(0, len(x_values)):
            rand = np.arange(len(x_values))
            np.random.shuffle(rand)
            x_values = x_values[rand]
            target = target[rand]
            rand = np.random.randint(0, len(x_values))
            output = feed_forward(x_values[rand].reshape(1, -1))
            backward(learning_rate, target[rand].reshape(1, -1), output[2], output)
