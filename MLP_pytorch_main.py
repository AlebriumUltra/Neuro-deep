...
X_test = df.iloc[100:150, 0:4].values
y = df.iloc[100:150, 4]
y = y.map({'Iris-setosa': 1, 'Iris-virginica': 2, 'Iris-versicolor':3}).values.reshape(-1,1)
...
net = DFF(inputSize,hiddenSizes, outputSize)
lossFn = nn.MSELoss()
...
loss_ = train(torch.from_numpy(X.astype(np.float32)), 
              torch.from_numpy(Y.astype(np.float32)), 3000)

# for name, param in net.named_parameters():
#     print(name, param)

pred = net.feed_forward(torch.from_numpy(X.astype(np.float32))).detach().numpy()
err = sum(abs((pred>0.5)-Y))
print(err)   

pred = net.feed_forward(torch.from_numpy(X_test.astype(np.float32))).detach().numpy()
